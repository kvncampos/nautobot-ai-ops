...
(function () {
  ...
  function loadChatHistory() {
      ...
      if (saved) {
        ...
        if (chatHistory.length < originalLength && chatHistory.length === 0) {
          clearChatWithMessage(
            `Previous conversation expired (older than ${CHAT_TTL_MINUTES} minutes). ` +
    ...
    if (chatHistory.length === 0) {
      ...
      const chatEnabled =
        window.CHAT_ENABLED !== undefined ? window.CHAT_ENABLED : true;
      if (chatEnabled) {
        addMessage(
          "Welcome to Nautobot GPT! I can help you query and interact with Nautobot APIs. Type a message to get started.",
          "ai",
        );
      } else {
        // Determine what's missing and show appropriate error message
        const hasDefaultModel =
          window.HAS_DEFAULT_MODEL !== undefined
            ? window.HAS_DEFAULT_MODEL
            : true;
        let errorMessage = "Chat is currently disabled. ";
        if (!hasDefaultModel) {
          errorMessage +=
            "Please configure a default LLM model to enable the AI Chat Agent.";
        } else {
      ...
      if (typeof window.DOMParser !== "undefined") {
        ...
        codes.forEach((code) => {
          ...
          const looksLikeTable =
            hasPipe && (hasSeparator || /^\s*\|/.test(codeText));
          const isMermaid =
            className.indexOf("language-mermaid") !== -1 ||
            className.indexOf("lang-mermaid") !== -1;
  ...
  function resetInactivityTimer() {
    ...
    inactivityTimer = setTimeout(() => {
      ...
      clearChatWithMessage(
        `👋 Hey there! Your chat history was cleared after ${CHAT_TTL_MINUTES} minutes of inactivity, but I'm still here and ready to help. How can I assist you?`,
      );
    }, INACTIVITY_TIMEOUT);
  }
  ...
  async function sendMessage() {
    ...
    sendButton.innerHTML =
      '<i class="mdi mdi-loading mdi-spin"></i> Thinking...';
    ...
    try {
      ...
      if (error.name === "AbortError") {
        ...
        addMessage(
          `ERROR: Failed to communicate with server: ${error.message}`,
          "ai",
          true,
        );
      }
  ...
  async function clearChatWithMessage(message) {
    ...
    clearButton.innerHTML =
      '<i class="mdi mdi-loading mdi-spin"></i> Stopping...';
  ...
  function clearChat() {
    if (confirm("Are you sure you want to clear the chat history?")) {
      clearChatWithMessage("Chat history cleared.\nHow can I help you today?");
    }
  }
  ...
  loadMarkedScript()
    .catch((err) => {
      console.warn(
        "Marked.js not available, messages will use plain text fallback.",
        err,
      );
    })
    .finally(() => {
