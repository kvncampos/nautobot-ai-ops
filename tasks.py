...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_AI_OPS_xxx
namespace = Collection("ai_ops")
namespace.configure(
    {
        "ai_ops": {
            "nautobot_ver": "3.0.3",
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.ai_ops.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.ai_ops.compose_http_timeout,
        "NAUTOBOT_VER": context.ai_ops.nautobot_ver,
        "PYTHON_VER": context.ai_ops.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.ai_ops.project_name}",
        f'--project-directory "{context.ai_ops.compose_dir}"',
    ]

    for compose_file in context.ai_ops.compose_files:
        compose_file_path = os.path.join(context.ai_ops.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.ai_ops.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.ai_ops.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.ai_ops.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.ai_ops.compose_dir, "creds.example.env"),
            os.path.join(context.ai_ops.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.ai_ops.python_ver
    dockerfile_path = os.path.join(context.ai_ops.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.ai_ops.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.ai_ops.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.ai_ops.python_ver}"
            run_command(context, command)
    else:
...
@task(
    help={
        "start_migration": "Starting migration number (e.g., 0001)",
        "end_migration": "Ending migration number (e.g., 0003)",
    }
)
def squashmigrations(context, start_migration="", end_migration=""):
    """Squash migrations for ai_ops app into a single migration."""
    if not start_migration or not end_migration:
        raise ValueError("Both start_migration and end_migration are required (e.g., 0001 0003)")

    command = f"nautobot-server squashmigrations ai_ops {start_migration} {end_migration}"
    run_command(context, command)


@task(
    help={
        "app": "App name to fake migrate (default: ai_ops)",
        "migration": "Migration to fake to (default: zero to unapply all)",
    }
)
def fake_migrate(context, app="ai_ops", migration="zero"):
    """Fake migrate to mark migrations as applied/unapplied without running them."""
    command = f"nautobot-server migrate --fake {app} {migration}"
    run_command(context, command)
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations ai_ops"
...
def pylint(context):
    ...
    command = f"{base_pylint_command} ai_ops"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("ai_ops") / Path("migrations")
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 ai_ops/templates/"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.ai_ops.local):
